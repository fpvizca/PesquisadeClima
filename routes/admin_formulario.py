...
def init_routes(app):
    ...
    def admin_formulario():
        ...
        return render_template('admin_formulario.html', dados=dados)

    @app.route('/admin/formulario/secao/nova', methods=['GET', 'POST'])
    @login_required
    def admin_secao_nova():
        if not has_role(session['usuario_id'], 'admin'):
            flash('Acesso negado.', 'danger')
            return redirect(url_for('pesquisa'))
        ...
        if request.method == 'POST':
            ...
            if not nome:
                ...
                return redirect(url_for('admin_secao_nova'))

            db.execute(
                "INSERT INTO secoes (nome, descricao, ordem) VALUES (?, ?, ?)",
                (nome, descricao if descricao else None, ordem)
            )
            db.commit()
            flash('Seção criada com sucesso!', 'success')
            return redirect(url_for('admin_formulario'))

        max_ordem = db.execute("SELECT COALESCE(MAX(ordem), 0) as m FROM secoes").fetchone()['m']
        return render_template('admin_secao_form.html', secao=None, max_ordem=max_ordem)
    ...
    def admin_secao_editar(secao_id):
        ...
        return render_template('admin_secao_form.html', secao=secao, max_ordem=0)
    ...
    def admin_secao_duplicar(secao_id):
        ...
        c = db.execute(
            "INSERT INTO secoes (nome, descricao, ordem) VALUES (?, ?, ?)",
            (secao['nome'] + ' (Cópia)', secao['descricao'], max_ordem + 1)
        )
    ...
    @app.route('/admin/formulario/pergunta/nova', methods=['GET', 'POST'])
    @login_required
    def admin_pergunta_nova():
        ...
        if request.method == 'POST':
            ...
            if not secao_id or not texto:
                ...
                return redirect(url_for('admin_pergunta_nova'))
        ...
        return render_template('admin_pergunta_form.html', pergunta=None, secoes=secoes, secao_id=secao_id, max_ordem=max_ordem)
    ...
    def admin_pergunta_editar(pergunta_id):
        ...
        return render_template('admin_pergunta_form.html', pergunta=pergunta, secoes=secoes, secao_id=pergunta['secao_id'], max_ordem=0)
